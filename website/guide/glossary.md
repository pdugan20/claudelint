...
An object passed to a rule's `validate()` function containing file information and reporting methods.
...
interface ValidationContext {
  ...
  options: unknown; // Rule-specific options from config
  report: (violation: Violation) => void; // Report violations
}
```
