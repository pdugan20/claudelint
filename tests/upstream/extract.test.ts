...
describe('assertMinFacts', () => {
  ...
  it('throws when upstream reformats a table into prose', () => {
    ...
    expect(() => assertMinFacts('hooks', events, 25)).toThrow(
      /hooks yielded \d+ facts, expected >= 25/
    );
  });
});

describe('linked event table rows', () => {
  it('extracts linked identifiers after upstream restyles a table', () => {
    expect(extractHookEvents('| [`DirectoryAdded`](#directoryadded) | Event |')).toEqual([
